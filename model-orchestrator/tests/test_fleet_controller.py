...
class FleetControllerTests(unittest.TestCase):
    ...
    def test_config_booleans_require_json_boolean_values(self):
        fleet = load_fleet()

        invalid_configs = []
        for value in ("false", "true", 0, 1, None):
            invalid_configs.append(config_for(allow_unknown_quota=value))
            config = config_for()
            config["hosts"][0]["agents"][0]["enabled"] = value
    ...
    def test_empty_command_arrays_are_rejected_before_execution(self):
        fleet = load_fleet()

        invalid_configs = []
        config = config_for()
        config["hosts"][0]["status_command"] = []
        invalid_configs.append(config)
        for command_key in ("drain_command", "resume_command", "fallback_command", "restore_command", "refresh_command"):
            config = config_for()
            config["hosts"][0]["agents"][0][command_key] = []
            invalid_configs.append(config)
    ...
    def test_string_boolean_policy_and_agent_values_are_rejected(self):
        fleet = load_fleet()

        for string_value in ("false", "False", "no", "0", "off", "FALSE", "true", "True", "yes", "1", "on"):
            with self.subTest(field="allow_unknown_quota", value=string_value):
                with self.assertRaisesRegex(ValueError, "allow_unknown_quota must be a JSON boolean"):
                    fleet.validate_config(config_for(allow_unknown_quota=string_value))

            with self.subTest(field="enabled", value=string_value):
                cfg = config_for()
                cfg["hosts"][0]["agents"][0]["enabled"] = string_value
