...
class CodexKeyringStatusSensorTests(unittest.TestCase):
    def test_sensor_preserves_supported_quota_timestamp_aliases(self):
        """checked_at from stats files must survive normalization as checkedAt."""
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            stats = root / "stats"
            stats.mkdir()
            (stats / "primary.json").write_text(
                json.dumps(
                    {
                        "alias": "primary-codex",
            ...
            payload = {
                ...
                "aliases": [{"alias": "primary-codex", "active": True}],
            }

            import importlib.util

            spec = importlib.util.spec_from_file_location("codex_keyring_status_sensor", SENSOR)
            assert spec is not None and spec.loader is not None
            sensor = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(sensor)
            rendered = sensor.sanitized_status(payload, stats)

        self.assertEqual(rendered["aliases"][0]["checkedAt"], "2026-08-07T12:00:00Z")
        self.assertNotIn("checked_at", rendered["aliases"][0])

    def test_sensor_preserves_all_supported_quota_timestamp_fields(self):
        """All alias-level timestamp fields recognized by the controller must
        survive the sensor allowlist (snake_case normalized to camelCase)."""
        timestamp_fields = {
            "quotaObservedAt": "2026-08-07T12:00:00Z",
            "quotaCheckedAt": "2026-08-07T12:01:00Z",
            "checkedAt": "2026-08-07T12:02:00Z",
            "statusCheckedAt": "2026-08-07T12:03:00Z",
            "observedAt": "2026-08-07T12:04:00Z",
        }
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            ...
            keyring.write_text(
                "#!/usr/bin/env python3\n"
                "import json\n"
                f"print(json.dumps({{'state': {{}}, 'aliases': [{{'alias': 'ts-account', **{json.dumps(timestamp_fields)}}}]}}))\n"
            )
            keyring.chmod(0o755)

            proc = subprocess.run(
                [sys.executable, str(SENSOR), "--keyring-binary", str(keyring), "--stats-dir", str(stats), "--json"],
                cwd=str(ROOT.parent),
                env=dict(os.environ),
                text=True,
        ...
        entry = payload["aliases"][0]
        for camel_field, expected_value in timestamp_fields.items():
            self.assertEqual(
                entry.get(camel_field),
                expected_value,
                f"sensor must preserve {camel_field}, got {entry.get(camel_field)}",
            )
