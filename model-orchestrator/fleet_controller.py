...
def _ensure_json_bool(value: Any, name: str) -> None:
...
def validate_config(config: Mapping[str, Any]) -> None:
    ...
    if "allow_unknown_quota" in policy:
        _ensure_json_bool(policy["allow_unknown_quota"], "allow_unknown_quota")
    ...
    for host in hosts:
        ...
        for configured_agent in agents:
            ...
            if "enabled" in configured_agent:
                _ensure_json_bool(configured_agent["enabled"], f"enabled for agent on host {host_id}")
            if not _bool_value(configured_agent.get("enabled", True), True):
                continue
...
def active_status_block_reason(active_alias: str | None, account: AccountQuota | None, config: Mapping[str, Any]) -> str | None:
    allow_unknown = _bool_value(policy_value(config, "allow_unknown_quota", False), False)
    if not active_alias:
...
def plan_fleet(config: Mapping[str, Any], statuses: Mapping[str, Any], state: Mapping[str, Any] | None = None, now: str | None = None) -> dict[str, Any]:
    ...
    for host in config.get("hosts", []):
        ...
        host_state = ((state.get("hosts") or {}).get(host_id) or {}) if isinstance(state, Mapping) else {}
        previous_mode = host_state.get("mode", "codex")
        agents = [agent for agent in (host.get("agents") or []) if _bool_value(agent.get("enabled", True), True)]
